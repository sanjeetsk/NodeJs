...
const server = http.createServer((req, res) => {
  ...
  if (req.method === "POST") {
    ...
    req.on("end", () => {
      ...
      fs.appendFile('queries.txt', `Name: ${name}\n, Email: ${email}\n, Message: ${message}\n\n`, (err) => {
        if (err) throw err;
        console.log('Data appended to queries.txt');
      });
      ...
      transporter.sendMail(mailOptions, (error, info) => {
        if (error) {
          ...
          // Emit a custom event when email is sent successfully
          eventEmitter.emit('mailSent', email);
        }
      });
